...
public enum ElementType {
    ...
    SOLAR("solar", ParticleTypes.FLAME, 25f, 100, 0.01f, 0xFFFF9A3C),
    /** 电弧：标记型，受击时 Jolt 连锁闪电（连锁目标致盲），击杀带电弧标记目标获得强化（Amplified） */
    ARC("arc", ParticleTypes.ELECTRIC_SPARK, 1f, 100, 0f, 0xFF5CC8FF),
    /** 虚空：标记型，受击时 Volatile 爆炸（10% 最大生命 AOE）+ 期间受击增伤（Weaken），击杀回血（Devour） */
    VOID("void", ParticleTypes.SCULK_SOUL, 1f, 100, 0f, 0xFF9B59B6),
    /** 冰影：命中 +50 层，渐进减速（≥50 缓慢 I / ≥75 缓慢 II），满 100 冻结，冻结中受击 Shatter +50% */
    STASIS("stasis", ParticleTypes.SNOWFLAKE, 50f, 100, 0f, 0xFF7FD8E6),
    /** 缚丝：叠层（+25/击），期间攻击者伤害 -40%（Sever），满 100 悬挂（Suspend），击杀带标记目标获得织甲（Woven Mail） */
    STRAND("strand", ParticleTypes.ENCHANT, 25f, 100, 0f, 0xFF8FDB6A),
    /** 棱镜：标记型（+1），受击时折射（Refract）——本击 25% 伤害溅射周围；棱镜攻击对所有元素护盾按匹配效率破盾（折射所有光 = 匹配所有盾） */
    PRISM("prism", ParticleTypes.FIREWORK, 1f, 100, 0f, 0xFFA78BFA);
    ...
    /** 命运2 元素色（HUD/护盾/词条显示统一使用） */
    private final int color;

    ElementType(String id, ParticleOptions particle, float stacksPerHit, int stateDuration, float doTPerStack, int color) {
        this.id = id;
